(ns nutrimat.zops
  (:require [nutrimat.binary :as binary]
            [nutrimat.zins :as zins :refer [defop]]
            [nutrimat.zio :as zio]
...
(defop je :op2 1 nil #{:branch} "je a b ?(label)"
  ...
  (let [a (zins/get-operand-value (ops 0))
        b (zins/get-operand-value (ops 1))]
    (zmachine/jump-if (= a b) branch)))
...
(defop dec_chk :op2 4 nil #{:branch} "dec_chk (variable) value ?(label)"
  ...
  (let [var (second (ops 0))
        value (zins/get-operand-value (ops 1))]
    (zmachine/dec-variable var)
    (zmachine/jump-if (< (zmachine/get-variable var) value) branch)))

(defop inc_chk :op2 5 nil #{:branch} "inc_chk (variable) value ?(label)"
  [ops store branch text]
  (let [var (second (ops 0))
        value (zins/get-operand-value (ops 1))]
    (zmachine/inc-variable var)
    (zmachine/jump-if (> (zmachine/get-variable var) value) branch)))

(defop jin :op2 6 nil #{:branch} "jin obj1 obj2 ?(label)"
  [ops store branch text]
  (let [obj1 (zins/get-operand-value (ops 0))
        obj2 (zins/get-operand-value (ops 1))]
    (zmachine/jump-if (= (zobj/object-parent obj1) obj2) branch)))
...
(defop and :op2 9 nil #{:store} "and a b -> (result)"
  ...
  (let [a (zins/get-operand-value (ops 0))
        b (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (bit-and a b))))

(defop test_attr :op2 10 nil #{:branch} "test_attr object attribute ?(label)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        attr (zins/get-operand-value (ops 1))]
    (zmachine/jump-if (zobj/object-attribute? obj attr) branch)))

(defop set_attr :op2 11 nil #{} "set_attr object attribute"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        attr (zins/get-operand-value (ops 1))]
    (zobj/set-object-attribute obj attr)))

(defop clear_attr :op2 12 nil #{} "clear_attr object attribute"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        attr (zins/get-operand-value (ops 1))]
    (zobj/clear-object-attribute obj attr)))

(defop store :op2 13 nil #{} "store (variable) value"
  [ops store branch text]
  (let [var (second (ops 0))
        value (zins/get-operand-value (ops 1))]
    (zmachine/set-variable var value)))

(defop insert_obj :op2 14 nil #{} "insert_obj object destination"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        parent (zins/get-operand-value (ops 1))]
    (zobj/object-insert obj parent)))

(defop loadw :op2 15 nil #{:store} "loadw array word-index -> (result)"
  [ops store branch text]
  (let [address (zins/get-operand-value (ops 0))
        word-index (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (zmem/zword (+ address (* word-index 2))))))

(defop loadb :op2 16 nil #{:store} "loadb array byte-index -> (result)"
  [ops store branch text]
  (let [address (zins/get-operand-value (ops 0))
        byte-index (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (zmem/zbyte (+ address byte-index)))))

(defop get_prop :op2 17 nil #{:store} "get_prop object property -> (result)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        prop (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (zobj/object-property obj (dec prop)))))

(defop get_prop_addr :op2 18 nil #{:store}
  "get_prop_addr object property -> (result)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        prop (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (zobj/object-property-addr obj (dec prop)))))
...
(defop get_next_prop :op2 19 nil #{:store}
  ...
  (let [obj (zins/get-operand-value (ops 0))
        prop (zins/get-operand-value (ops 1))]
    (zmachine/set-variable store (zobj/object-next-prop obj (dec prop)))))

(defop add :op2 20 nil #{:store} "add a b -> (result)"
  [ops store branch text]
  (let [a (binary/signed-word (zins/get-operand-value (ops 0)))
        b (binary/signed-word (zins/get-operand-value (ops 1)))]
    (zmachine/set-variable store (+ a b))))
...
(defop div :op2 23 nil #{:store} "div a b -> (result)"
  ...
  (let [a (binary/signed-word (zins/get-operand-value (ops 0)))
        b (binary/signed-word (zins/get-operand-value (ops 1)))]
    (zmachine/set-variable store (int (/ a b)))))
...
(defop jz :op1 0 nil #{:branch} "jz a ?(label)"
  [ops store branch text]
  (zmachine/jump-if (zero? (zins/get-operand-value (ops 0))) branch))

(defop get_sibling :op1 1 nil #{:branch :store}
  "get_sibling object -> (result) ?(label)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        sibling (zobj/object-parent obj)]
    (zmachine/set-variable store sibling)
    (zmachine/jump-if (not (zero? sibling)) branch)))

(defop get_child :op1 2 nil #{:branch :store}
  "get_child object -> (result) ?(label)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))
        parent (zobj/object-parent obj)]
    (zmachine/set-variable store parent)
    (zmachine/jump-if (not (zero? parent)) branch)))

(defop get_parent :op1 3 nil #{:store} "get_parent object -> (result)"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))]
    (zmachine/set-variable store (zobj/object-parent obj))))

(defop get_prop_len :op1 4 nil #{:store}
  "get_prop_len property-address -> (result)"
  [ops store branch text]
  (let [prop-addr (zins/get-operand-value (ops 0))]
    (zmachine/set-variable store (zobj/object-property-len prop-addr))))
...
(defop remove_obj :op1 9 nil #{} "remove_obj object"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))]
    (zobj/object-remove obj)))

(defop print_obj :op1 10 nil #{} "print_obj object"
  [ops store branch text]
  (let [obj (zins/get-operand-value (ops 0))]
    (zio/zprint (zobj/object-description obj))))

(defop ret :op1 11 nil #{} "ret value"
  [ops store branch text]
  (zmachine/routine-return (zins/get-operand-value (ops 0))))

(defop jump :op1 12 nil #{} "jump ?(label)"
  [ops store branch text]
  (let [offset (binary/signed-word (zins/get-operand-value (ops 0)))]
    (zmachine/set-pc (+ (zmachine/pc) offset -2))))
...
(defop rtrue :op0 0 nil #{} "rtrue"
  [& rest]
  (zmachine/return-true))

(defop rfalse :op0 1 nil #{} "rfalse"
  [& rest]
  (zmachine/return-false))
...
(defop ret_popped :op0 8 nil #{} "ret_popped"
  [ops store branch text]
  (zmachine/routine-return (zmachine/stack-pop)))
...
(defop call :var 0 [1 2 3] #{:store}
  "call routine ...0 to 3 args... -> (result)"
  [ops store branch text]
  (op-call ops store))

(defop call_vs :var 0 [4 5 6 7 8] #{:store}
...
(defop storew :var 1 nil #{} "storew array word-index value"
  ...
  (let [address (zins/get-operand-value (ops 0))
        word-index (zins/get-operand-value (ops 1))
        value (zins/get-operand-value (ops 2))]
    (zmem/set-zword (+ address (* word-index 2)) value)))
...
(defop storeb :var 2 nil #{} "storeb array byte-index value"
  ...
  (let [address (zins/get-operand-value (ops 0))
        byte-index (zins/get-operand-value (ops 1))
        value (zins/get-operand-value (ops 2))]
    (zmem/set-zbyte (+ address byte-index) value)))
...
(defop call_vs2 :var 12 [4 5 6 7 8] #{:store}
...
(defop sound_effect :var 21 [5 6 7 8] #{}
  "sound_effect number effect volume routine")

(defop read_char :var 22 [4 5 6 7 8] #{:store}
  "read_char 1 time routine -> (result)")

(defop scan_table :var 23 [4 5 6 7 8] #{:branch :store}
...
(defop picture_data :ext 6 [6 7 8] #{:branch}
...
(defop check_unicode :ext 12 [5 6 7 8] #{}
...
(defop get_wind_prop :ext 19 [6 7 8] #{:store}
...
(defop put_wind_prop :ext 25 [6 7 8] #{}
